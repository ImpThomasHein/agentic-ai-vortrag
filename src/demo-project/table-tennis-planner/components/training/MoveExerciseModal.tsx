...
interface MoveExerciseModalProps {
  ...
  onMove: (newDateString: string) => void;
}
...
export function MoveExerciseModal({
  ...
  const handleMove = (dateString: string) => {
    onMove(dateString);
    onClose();
  };
        ...
        <Card variant="glass" padding="none" className="overflow-hidden">
          ...
          <div className="p-4 max-h-[50vh] overflow-y-auto">
            <p className="text-xs font-medium mb-3" style={{ color: 'var(--text-secondary)' }}>
              Wähle einen neuen Trainingstag:
            </p>
            <div className="space-y-2">
              {availableDays.map((day) => {
                ...
                return (
                  <button
                    key={day.dateString}
                    onClick={() => !isCurrent && handleMove(day.dateString)}
                    disabled={isCurrent}
                    className={`
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {day.exerciseIds.length > 0 && !isCurrent && (
